...
//get total rows
$rs = $conn->query("select count(type_id) as num from tbl_type $sql_search "); // query แบบมีเงื่อนไข ถ้ามีการส่งค่าค้นหา
...
<table class="table table-striped shadow  mb-3 ">
    ...
    <tr>
        ...
        <td bgcolor="linear-gradient(90deg, #020024 0%, #090979 35%, #00d4ff 100%)" style="color: white;">หมวดหมู่</td>
    ...
    ?>
            <tr>
                ...
                <td> <a href="#" onclick="edit(<?php echo $row['type_id']; ?>);" data-toggle="modal" data-target="#dataModal"  class='btn btn-outline-warning btn-lg font1 shadow p-1 ' style="width: 50px;height: 38px;"><i class='fa fa-cog'></i> </a></td>
                <td> <a href="#" onclick="javascript:del('<?php echo $row['type_id']; ?>');" class='btn btn-outline-danger btn-lg shadow p-1' style="width: 50px;height: 38px;"><i class="fa fa-bitbucket"></i> </a></td>
            </tr>
